...
#endif
{
 ...
 protected:
  ...
  /// @brief Requests recurrent-state/partial-feature resets for every model. Safe from either component task.
  void request_model_state_resets_();

  /// @brief Clears probabilities and requests state/partial-feature resets. Called only by the inference task.
  void reset_model_contexts_();
