...
// Modified 2026-09-06 by the Pippa wake-word project to reset recurrent model context at stream boundaries.
...
#ifdef PIPPA_CRNN_METRICS
        this->crnn_ring_full_events_.fetch_add(1, std::memory_order_relaxed);
#endif
    ...
    if (!(xEventGroupGetBits(this_mww->event_group_) & ERROR_BITS)) {
      ...
      while (!(xEventGroupGetBits(this_mww->event_group_) & (COMMAND_STOP | ERROR_BITS))) {
        ...
        if (xEventGroupGetBits(this_mww->event_group_) & EventGroupBits::COMMAND_RESET_RING_BUFFER) {
          ...
          FrontendReset(&this_mww->frontend_state_);
          this_mww->reset_model_contexts_();
        ...
        while (audio_source->available() >= sizeof(int16_t)) {
          if (xEventGroupGetBits(this_mww->event_group_) & EventGroupBits::COMMAND_RESET_RING_BUFFER) {
            // A producer-side drop is a hard stream boundary. Return to the outer loop before touching any audio that
            // may have been appended after the gap; it will clear the ring and all inference context.
            break;
          }
          ...
          if (xEventGroupGetBits(this_mww->event_group_) & EventGroupBits::COMMAND_RESET_RING_BUFFER) {
            // The drop may have raced feature generation. Discard that feature; the outer-loop reset clears the
            // frontend window, partial model input, recurrent state, and probability history before resuming.
            break;
          }
...
void MicroWakeWord::loop() {
  ...
  for (auto *model : this->wake_word_models_) {
    ...
    if (model->take_crnn_metrics_snapshot(&metrics)) {
      ESP_LOGI(TAG,
               "CRNN_PERF model=%s n=%" PRIu32 " total=%" PRIu64 " runtime_budget_us=%" PRIu32
               " p50_bucket_us=%" PRIu32 " p95_bucket_us=%" PRIu32 " p99_bucket_us=%" PRIu32
               " runtime_max_us=%" PRIu32 " runtime_budget_overruns=%" PRIu32 " cold_n=%" PRIu32
               " cold_last_us=%" PRIu32 " cold_max_us=%" PRIu32 " cold_runtime_budget_overruns=%" PRIu32
               " cadence_interval_n=%" PRIu32 " cadence_threshold_us=%" PRIu32
               " cadence_max_gap_us=%" PRIu32 " cadence_gap_over_threshold=%" PRIu32
               " percentile_censored_mask=%u dropped_reports=%" PRIu32 " ring_full_total=%" PRIu32,
               model->get_id().c_str(), metrics.samples, metrics.total_samples, metrics.runtime_budget_us,
               metrics.p50_bucket_us, metrics.p95_bucket_us, metrics.p99_bucket_us, metrics.runtime_max_us,
               metrics.runtime_budget_overruns, metrics.cold_samples, metrics.cold_last_us, metrics.cold_max_us,
               metrics.cold_runtime_budget_overruns, metrics.cadence_intervals, metrics.cadence_threshold_us,
               metrics.cadence_max_gap_us, metrics.cadence_gap_over_threshold,
               metrics.percentile_censored_mask, metrics.dropped_reports,
               this->crnn_ring_full_events_.load(std::memory_order_relaxed));
    }
  }
#endif
...
void MicroWakeWord::start() {
  ...
  this->request_model_state_resets_();
    ...
    if (model->get_unprocessed_probability_status()) {
      ...
      if (wake_word_state.detected) {
        ...
        if (vad_state.detected) {
          ...
          // A wake accepted from any model is a stream boundary for every model that remains active.
          this->reset_model_contexts_();
          return;  // Emit at most one accepted wake from a single inference frame.
#ifdef USE_MICRO_WAKE_WORD_VAD
        } else {
...
void MicroWakeWord::request_model_state_resets_() {
  for (auto *model : this->wake_word_models_) {
    model->reset_streaming_state();
  }
#ifdef USE_MICRO_WAKE_WORD_VAD
  this->vad_model_->reset_streaming_state();
#endif
}

void MicroWakeWord::reset_model_contexts_() {
  for (auto *model : this->wake_word_models_) {
...
bool MicroWakeWord::update_model_probabilities_(const int8_t audio_features[PREPROCESSOR_FEATURE_SIZE]) {
  ...
  for (auto &model : this->wake_word_models_) {
    // Perform inference
    success = success & model->perform_streaming_inference(audio_features, this->features_step_size_ * 1000U);
  }
#ifdef USE_MICRO_WAKE_WORD_VAD
  success = success & this->vad_model_->perform_streaming_inference(audio_features, this->features_step_size_ * 1000U);
#endif
